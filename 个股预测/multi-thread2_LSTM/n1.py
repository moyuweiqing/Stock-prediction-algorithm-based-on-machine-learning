...
my_df = pd.DataFrame(np.arange(0.0, 300.0).reshape(300, 1),columns=['a'])
def getLastDay():
    for i in range(0, 300):
        temp = ts.get_hist_data(hs300['code'].loc[i], start='2019-01-02', end='2019-08-30')
        temp = temp.sort_index(ascending=True)
        my_df['a'].iloc[i] = temp['close'].iloc[-1]
    my_df.to_csv('lastday.csv')
